...
export class BanScanIterator implements Iterator<XY>, Iterable<XY> {
  ...
  valid() {
    return (
      this.ban.isLegal(this.position) &&
      this.underJumpLimit() &&
  ...
  jumpException() {
    ...
    return (
      nowkoma.isNull() || this.jumpExceptions.indexOf(nowkoma.species) == -1
    );
  }
...
class LoopBanScanIterator extends BanScanIterator {
  nextPlace() {
    this.position.addRel(this.vector);
    if (
      this.iteratorOption["only"] &&
      this.iteratorOption["only"].indexOf(
        this.ban.get(this.initPosition).species
      ) == -1
    )
      return;
...
class EpoxyBanScanIterator extends BanScanIterator {
  ...
  count() {
    ...
    if (
      this.ban.exists(this.position) &&
      this.ban.get(this.position).direction != 9
    ) {
