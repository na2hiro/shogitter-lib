    ...
    if (this.setting.fatal) {
      ...
      this.ban.checkOute().forEach((value, direction) => {
        ...
        this.ban.parent.gameEnd(
          ...
          win,
          "勝ち",
          `${this.ban.parent.teban.getName(
            direction
          )}玉に王手がかかりました。${this.ban.parent.teban.getName(
            win
          )}の勝ちです。`
        );
        cnt++;
      ...
      if (cnt > 1)
        throw new ShogitterCoreException("複数の玉に王手がかかっています。");
    } else {
...
class TsumiJudgeStrategy extends JudgeStrategy<TsumiJudgeConfig> {
    ...
    for (let player of this.ban.parent.teban.getIterator()) {
      ...
      if (!this.setting.uchifu && put && koma.species == "aa") {
        this.ban.parent.gameEnd(
          ...
          player,
          "反則勝ち",
          `打ち歩詰めです。${this.ban.parent.teban.getName(player)}の勝ちです。`
        );
      } else {
        this.ban.parent.gameEnd(
          player,
          ...
          `${this.ban.parent.teban.getName(
            player
          )}玉が詰みました。${this.ban.parent.teban.getName(
            koma.direction
          )}の勝ちです。`
        );
      }
  ...
  isTsumi(direction: Direction) {
    ...
    if (!this.ban.checkOuteByDirection(direction)) {
      if (debug)
        console.log(
          `王手でない${this.ban.get(new XY(5, 3))}${this.ban.get(new XY(5, 2))}`
        );
      return false;
    }
    ...
    for (let placeOu of arrayPlaceOu) {
      ...
      if (mapKiki["enemy"][placeOu.x]?.[placeOu.y]?.length > 0) {
        for (let placeFrom of mapKiki["enemy"][placeOu.x][placeOu.y]) {
          ...
          if (mapKiki["friend"][placeFrom.x]?.[placeFrom.y]?.length > 0) {
            for (let placeFromFrom of mapKiki["friend"][placeFrom.x][
              ...
              try {
                ...
                if (debug)
                  console.log(
                    `${placeFromFrom}から${placeFrom}へ移動すれば良い`
                  );
                return false;
              } catch (e) {}
            }
          ...
          //王手をしている駒と王の全ての間について
          for (let placeBetween of BanScanIterator.getBetween(
            this.ban,
            placeFrom,
            placeOu
          )) {
            ...
            if (
              ...
              for (let placeBetweenFrom of mapKiki["friend"][placeBetween.x][
                ...
                try {
                  ...
                  if (debug)
                    console.log(
                      `${placeBetweenFrom}から${placeBetween}へ移動すれば良い`
                    );
                  return false;
                } catch (e) {}
              }
  ...
  execute(to: XY) {
    ...
    if (!this.ban.canOthello()) {
      ...
      this.ban.parent.gameEnd(
        ...
        mark,
        meskifu,
        `${point[0] || 0}対${point[1] || 0}で${mes}です`
      );
    }
  }
  toHTML() {
...
class GomokuJudgeStrategy extends JudgeStrategy<GomokuJudgeConfig> {
  ...
  execute(to: XY) {
    ...
    if (this.setting["sansan"] && this.sansan(to)) {
      ...
      this.gameEnd(
        ...
        win,
        "反則負け",
        `三三です。${this.ban.parent.teban.getName(lose)}の負けです。`
      );
      ...
      this.gameEnd(
        ...
        `${num2kan_decimal(
          this.setting["moku"]
        )}目並びました。${this.ban.parent.teban.getName(win)}の勝ちです。`
      );
    } else {
      for (let koma of this.ban.getIterator()) {
  ...
  sansan(to: XY) {
    ...
    outer: for (let relXY of RelXY.getVector(RelativeType.HALFROUND)) {
      ...
      values.push(
        Math.max(hanare[0], hanare[1]) + (centerikidomari == 1 ? 0 : center)
      );
    }
...
class IgoJudgeStrategy extends JudgeStrategy<{}> {
  ...
  execute(to: XY) {
    ...
    if (tesuu > 1) {
      if (
        this.ban.parent.getHash() == this.ban.parent.kifu.getHash(tesuu - 2)
      ) {
  ...
  execute(to: XY) {
    ...
    if (!this.ban.isLegal(koma.XY.getCloneRel(vec))) {
      ...
      this.ban.parent.gameEnd(
        ...
        win,
        "トライ",
        `トライしました。${this.ban.parent.teban.getName(win)}の勝ちです。`
      );
    }
  }
  toHTML() {
...
class CountJudgeStrategy extends JudgeStrategy<CountJudgeConfig> {
    ...
    for (const player of this.ban.parent.teban.getIterator()) {
      if (this.setting["lte"] && maisuu[player] <= this.setting["lte"]) {
        ...
        this.ban.parent.gameEnd(
          ...
          win,
          "勝ち",
          `${this.ban.parent.teban.getName(lose)}の駒が${
            this.setting["lte"]
          }枚以下になりました。${this.ban.parent.teban.getName(
            win
          )}の勝ちです。`
        );
        break;
      ...
      if (this.setting["gte"] && this.setting["gte"] <= maisuu[player]) {
        ...
        this.ban.parent.gameEnd(
          ...
          `${this.ban.parent.teban.getName(lose)}の駒が${
            this.setting["gte"]
          }枚以上になりました。${this.ban.parent.teban.getName(
            win
          )}の勝ちです。`
        );
        break;
  ...
  toHTML() {
    return `駒が${
      this.setting["lte"]
        ? this.setting["lte"] + "枚以下"
        : this.setting["gte"] + "枚以上"
    }になったら負け`;
  }
}
class NoMoveJudgeStrategy extends JudgeStrategy<{}> {
  ...
  execute(to: XY) {
    ...
    this.ban.parent.gameEnd(
      ...
      win,
      "勝ち",
      `動ける駒がなくなりました。${this.ban.parent.teban.getName(
        win
      )}の勝ちです。`
    );
  }
