...
class QuantumMoveEffectStrategy extends MoveEffectStrategy {
  ...
  executeAfter(to: XY, captured = false) {
    ...
    this.obj[pos.direction].d = runQuantum(
      pos.position,
      {
        vec: rel,
        promote: this.prom,
    ...
    if (captured) {
      ...
      const mochiIdx = this.countMochigoma(this.direction);
      this.obj[capturedPos.direction].xys[
        capturedPos.position
      ] = `mochi${this.direction}-${mochiIdx}`;
      this.obj[capturedPos.direction].d = runQuantum(
        capturedPos.position,
        null,
        MoveType.Captured,
      ...
      const definitelyOu = this.obj[capturedPos.direction].d.kinds.findIndex(
        (sp) => sp.length === 1 && sp[0] === "Ou"
      );
      if (
        definitelyOu >= 0 &&
        this.obj[capturedPos.direction].xys[definitelyOu].startsWith("mochi")
      ) {
        const capturedDirection = 1 - this.direction;
