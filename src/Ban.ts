...
export default class Ban {
    ...
    public moveFilter(arrayMovable: Kiki[], to: XY, direction: Direction, species: Species, flags: Flags): Kiki[] {
        return this.strategy.MoveFilter.execute(arrayMovable, to, direction, species, flags);
    }
    ...
    public getScanIterator(position: XY, vector: RelXY, flags: Flags = {}) {
        return BanScanIterator.create(this.banScanIterator['name'], this, position, vector, {
            ...flags,
    ...
    getDifference(ban: BanObj, to: XY, from: XY = null): MoveDiff[] {
        ...
        for (let x = 1; x <= this.x; x++) {
            for (let y = 1; y <= this.y; y++) {
                ...
                if ((last.length == 0 && now.length == 0) || (last[0] == now[0] && last[1] == now[1])) continue;
    ...
    makePostfix(from: XY, to: XY) {
        ...
        let flagRight, flagLeft, flagSameLR, flagUp, flagDown, flagSameUD, LRPostfix = "", noUD, UDPostfix = "", noLR;
    ...
    copy(to: XY, from: XY) {
        if (this.exists(from)) {
            ...
            this.arrayBan[to.x][to.y] = this.createKoma(to, fromkoma.species, fromkoma.direction);
        } else {
            this.remove(to);
        }
    }
    ...
    countSpecies() {
        ...
        for (let koma of this.getIterator()) {
            ...
            if (!point[koma.direction][koma.species]) point[koma.direction][koma.species] = 0;
    ...
    arrayKikiKoma(xy: XY, species: Species, direction: Direction) {
        ...
        const ret = [];
        for (let kiki of this.get(xy).getMovableVirtual(species,
            Teban.reverse(direction),
            {'directionCalcingAllowed': direction }))
        {
            if (this.getSpecies(kiki['XY']) == species && this.getDirection(kiki['XY']) == direction) {
                ret.push(kiki);
            }
        }
        return ret;
        ...
        //if (typeof direction === "undefined") {
            for (let koma of this.getIterator()) {
                ...
                for (let kiki of koma.getMovable()) {
                    ...
                    if(!ret[koma.direction == direction ? 'friend' : 'enemy']) ret[koma.direction == direction ? 'friend' : 'enemy'] = [];
                    if(!ret[koma.direction == direction ? 'friend' : 'enemy'][kiki['XY'].x]) ret[koma.direction == direction ? 'friend' : 'enemy'][kiki['XY'].x] = [];
                    if(!ret[koma.direction == direction ? 'friend' : 'enemy'][kiki['XY'].x][kiki['XY'].y]) ret[koma.direction == direction ? 'friend' : 'enemy'][kiki['XY'].x][kiki['XY'].y] = [];
                    ret[koma.direction == direction ? 'friend' : 'enemy'][kiki['XY'].x][kiki['XY'].y].push(koma.XY);
                }
            }
            /*
    ...
    existsMovable(xy: XY, direction: Direction, flag: Flags = {}) {
        for (let koma of this.getIterator()) {
            if (koma.isNull()) continue;
            if (!(!flag.igai && koma.direction == direction)
                && !(flag.igai && koma.direction != direction)) continue;//目的の向きではなかったら飛ばす
     ...
     * @return <type>
     */
    arrayKikiInSpeDir(species: Species, flagSpeExcept: boolean, direction: Direction, flagDirExcept: boolean, flagToruichi: boolean) {
        let kikiArray: Kiki[] = [];
        for (let koma of this.getIterator()) {
            if (koma.isNull()) continue;
            //flag0:向きが調べたい駒と違ったら flag1:向きが調べたくない駒と一致したら また、駒指定ある場合その駒と違ったら、continue
            if ((!flagDirExcept && koma.direction != direction)
                || (flagDirExcept && koma.direction == direction)
                || (species != ""
                    && ((!flagSpeExcept && koma.species != species)
                        || flagSpeExcept && koma.species == species))) continue;
...
///変則将棋
    ...
    getSurroundedByEnemy(xy: XY, arrayXY: XY[] = []): XY[] {
        ...
        for (let relXY of RelXY.getVector(RelativeType.TATEYOKO)) {
            ...
            //空白でない時
            if (this.getDirection(tan) == this.getDirection(xy)) {//隣が味方の場合
...
//エポキシ接着(縦横に相手の駒がいる状態)しているものを取得
    getEpoxied(xy: XY, arrayXY: XY[] = []): XY[] {
        ...
        for (let relXY of RelXY.getVector(RelativeType.TATEYOKO)) {
            ...
            if (this.getDirection(tan) !== this.getDirection(xy)) {//敵の場合
