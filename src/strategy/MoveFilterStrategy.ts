...
export class MoveFilterStrategyContainer<S = {}> extends StrategyContainer<MoveFilterStrategy<S>>{
	...
	execute(arrayMovable, to: XY, direction: Direction, species: Species, flags){
		for(const strategy of this.arrayStrategies){
			arrayMovable=strategy.execute(arrayMovable, to, direction, species, flags);
		}
		return arrayMovable;
	}
...
export default abstract class MoveFilterStrategy<S = {}> extends Strategy{
	...
	abstract execute(arrayMovable: Kiki[], to: XY, direction: Direction, species: Species, flags: Flags): Kiki[];
...
class RestrictedMoveFilterStrategy extends MoveFilterStrategy<RestrictedMoveFilterConfig>{
	...
	execute(arrayMovable: Kiki[], to: XY, direction: Direction, species: Species, flags: Flags){
		...
		for(const place of this.setting['place']){
			...
			if(dir==0){
				...
				min={'x': this.ban.x-place['max']['x']+1,
						   'y': this.ban.y-place['max']['y']+1};
				max={'x': this.ban.x-place['min']['x']+1,
						   'y': this.ban.y-place['min']['y']+1};
			}
			arrayMovable=arrayMovable.filter(kiki => min['x']<=kiki['XY'].x && kiki['XY'].x<=max['x'] && min['y']<=kiki['XY'].y && kiki['XY'].y<=max['y']);
		}
	...
	toHTML() {
		let ret="";
		for(const place of this.setting['place']){
			ret+=` ${place['species'].map(sp => `<a href='/koma/{sp}'>${shogitterDB.getKoma(sp, "name")}</a>`)}は${place['min']['x']}〜${place['max']['x']}列${place['min']['y']}〜${place['max']['y']}段`;
		}
		ret+="のみ移動可能";
		return ret;
	}
}
const nameToStrategy: {[variant: string]: /*(typeof MoveFilterStrategy)*/any} = {
