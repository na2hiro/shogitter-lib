...
class ShareMochigomaIOStrategy extends MochigomaIOStrategy<{}> {
    public abstract = "持ち駒共有(取った駒は両方の駒台に乗り、片方が使うと両方の駒が消える)";
...
class BothFaceMochigomaIOStrategy extends MochigomaIOStrategy<{}> {
    public abstract = "駒をとると表裏の駒が駒台に乗り、どちらの面でも打つことが出来る。";
...
class ExchangeMochigomaIOStrategy extends MochigomaIOStrategy<ExchangeMochigomaIOConfig> {
    ...
    toHTML() {
        ...
        for (let dir in this.setting) {
            ...
            for (let from in s) {
                const to = s[from];
                ret += " " + Teban.tebanName[0][dir] + "の<a href='/koma/{from}'>" + shogitterDB.getKoma(from, "name") + "</a>は<a href='/koma/{to}'>" + shogitterDB.getKoma(to, "name") + "</a>に";
            }
        }
        return ret + "置き換えられる";
    }
