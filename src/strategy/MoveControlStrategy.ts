...
class FreezeMoveControlStrategy extends MoveControlStrategy<{}>{
	...
	isKanashibari(from: XY){
		if(this.ban.existsMovable(from, this.ban.get(from).direction, {'igai': true})){
			return true;
		}
		return false;
	}
...
class MadrasMoveControlStrategy extends MoveControlStrategy<{}>{
	...
	isMadorasi(from: XY){
		...
		for(const kiki of this.ban.arrayKikiInSpeDir(koma.species, false, koma.direction, true, false)){
...
class VolleyballMoveControlStrategy extends MoveControlStrategy<{}>{
	...
	isVolleyball(from: XY){
		...
		switch(tesuu%6){
			...
			case 2:
				//２手前を探す
				bad=this.ban.parent.kifu.getXYByTesuu(this.ban.parent.kifu.getTesuu()-2);
			...
			case 1:
				//１手前を探す
				bad=this.ban.parent.kifu.getXYByTesuu(this.ban.parent.kifu.getTesuu()-1);
				if(from.equals(bad['to'])){
					return false;
				}
			case 3:
...
class NoTaimenMoveControlStrategy extends MoveControlStrategy<NoTaimenMoveControlConfig>{
	...
	executeAfter(to: XY){
		...
		for(const koma of this.ban.getIterator()){
			if(koma.isNull() 
			   ...
			   || this.setting['species'].indexOf(koma.species)===-1) continue;
	...
	toHTML() {
		return `${this.setting['species'].map(sp => `<a href='/koma/{sp}'>${shogitterDB.getKoma(sp, "name")}</a>`).join(" ")}どうしが対面してはいけない`;
	}
}
