...
class ToruichiCaptureControlStrategy extends CaptureControlStrategy<{}> {
    ...
    isCapturable(direction: Direction) {
        ...
        for (let koma of this.ban.getIterator()) {
            ...
            const flagWinkoma = this.ban.parent.rule['winkoma'].indexOf(koma.species) >= 0;
            for (let kiki of koma.getMovable()) {
                const nowkoma = this.ban.get(kiki['XY']);
                if (!nowkoma.isNull() && nowkoma.direction != direction) {//行き先に敵があったら
                    if (!flagWinkoma) {
    ...
    execute(captured: Koma, capturing: Koma) {
        ...
        if (this.isShishi(capturing.species)) {
            ...
            if (!this.ban.parent.moving) {
                //移動中でない
                if (Math.abs(capturing.XY.x - captured.XY.x) == 2 || Math.abs(capturing.XY.y - captured.XY.y) == 2) {
                ...
                tmp.rollback(1);
                if (!(tmp.ban.exists(last['to']) && this.canTsukegui(tmp.ban.getSpecies(last['to'])))) {
                    const xy = capturing.XY;
                    const picked = this.ban.cut(xy);
            ...
            //それ以外の駒でとる場合
            const tmp = this.ban.parent.kifu.getDataByTesuu(this.ban.parent.kifu.getTesuu() - 1);
...
const nameToStrategy: {[variant: string]: /*typeof CaptureControlStrategy)*/any} = {
