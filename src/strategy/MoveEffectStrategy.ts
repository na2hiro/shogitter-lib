...
class HasamiMoveEffectStrategy extends MoveEffectStrategy{
	...
	executeAfter(to: XY){
		...
		for(const relXY of RelXY.getVector(RelativeType.TATEYOKO)){
			...
			if(this.ban.exists(tonari) && this.ban.get(tonari).isEnemy(this.ban.get(to))){
...
class IgoMoveEffectStrategy extends MoveEffectStrategy{
	...
	executeAfter(to: XY){
		for(const relXY of RelXY.getVector(RelativeType.TATEYOKO)){
			...
			if(this.ban.exists(tonari) && this.ban.get(tonari).isEnemy(this.ban.get(to))){
				for(const value of this.ban.getSurroundedByEnemy(tonari)){
					this.ban.strategy['Capture'].execute(value, this.ban.get(to).direction);
				}
			}
		}
	}
	...
	canJump(direction: Direction): boolean{
		for(const koma of this.ban.getIterator()){
			...
			for(const kiki of koma.getMovable() ){
				if(Math.abs(koma.XY.x-kiki['XY'].x)==2 && Math.abs(koma.XY.y-kiki['XY'].y)==2){
					return true;
				}
			}
		}
...
class FlipPutMoveEffectStrategy extends MoveEffectStrategy<FlipPutMoveEffectConfig>{
	...
	toHTML() {
		return (this.setting.common.directions?`${this.setting.common.directions.map(dir => Teban.tebanName[0][dir]).join(" ")}は`:"")
			+`<a href='/koma/${this.setting["species"]}'>${shogitterDB.getKoma(this.setting["species"], "name")}</a>を相手の駒のまま打つ`;
	}
}
/**
 * はさみ天秤端落とし
...
class AntikirukeMoveEffectStrategy extends MoveEffectStrategy{
	...
	public executeAfter(to: XY, captured=false) {
		...
		const koma=this.ban.get(to);
		if(!koma)return;
		for(const xy of this.ban.getNearestInitPlace(koma.direction, koma.getPromoted(0), to)){
...
class QuantumMoveEffectStrategy extends MoveEffectStrategy{
	 ...
	 *   Captured: 取られた
	 */
	command(position: number, vec: RelXY=null, promote: boolean, movetype: MoveType, kinds: string[][]){
	...
	private loadLatestData () {
		this.obj = JSON.parse(JSON.stringify(this.ban.parent.kifu.getLatestData("quantum"))) || [
			{xys: [], d: {kinds: [], fulls: []}},
			{xys: [], d: {kinds: [], fulls: []}},
		]
	}
	...
	executeAfter(to: XY, captured=false) {
		...
		if(rel.x>2)rel.x=2;if(rel.x<-2)rel.x=-2;
		if(rel.y>2)rel.y=2;if(rel.y<-2)rel.y=-2;
		let pos = this.findPos(this.from) || this.allocatePos(this.direction);

		const dan = this.ban.calcDan(this.to, this.direction);
		const movetype: MoveType = this.promNow ? MoveType.Promote : (!this.prom && dan <= 2 ? [MoveType.NoPromote1, MoveType.NoPromote2][dan - 1] : MoveType.Normal);
		this.obj[pos.direction].xys[pos.position]=this.to.toString();
		this.obj[pos.direction].d=this.command(pos.position, rel, this.prom, movetype, this.obj[pos.direction].d.kinds);
		...
		if (captured) {
			...
			//王除外
			this.obj[capturedPos["direction"]]["xys"][capturedPos.position] = `mochi${this.direction}-${this.countMochigoma(this.direction)}`;
			if (!end) this.obj[capturedPos.direction].d = this.command(capturedPos.position, null, null, MoveType.Captured, this.obj[capturedPos.direction].d.kinds);
		}
	}
	executeDrop(to: XY, id: number) {
		...
		const dan = this.ban.calcDan(this.to, this.direction);
		if(dan<=2){
			this.obj[pos["direction"]]["d"]=this.command(pos["position"], null, this.prom, [MoveType.NoPromote1, MoveType.NoPromote2][dan - 1], this.obj[pos.direction].d.kinds);
		}
	}
}
const nameToStrategy: {[variant: string]: /*(typeof MoveEffectStrategy)*/any} = {
