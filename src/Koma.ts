...
export class Koma {
    ...
    constructor(species: Species = null, direction: Direction | "_" = null, xy: XY = null, ban: Ban = null) {
    ...
    getData(komaName: string, memberName: string) {
        ...
        if (initial && initial[memberName] && this.ban.parent.kifu.isInitial(this.XY)) {
    ...
    public getMovable(flags: Flags = {}): Kiki[] {
        if (this.ban.parent.rule.rensa) {
            ...
            const search = (xy: XY) => {
                ...
                for (const kiki of currentKoma.getMovableVirtual(currentKoma.species, currentKoma.direction, {'ignoreOu': flags.ignoreOu})) {
                    const kikiXY = kiki.XY;
                    if (arrayChecked.some(checked => checked.x == kikiXY.x && checked.y == kikiXY.y)) {
                        //チェック済みリストに入っていたらそこは調べない。
                        continue;
                    }
                    if (!returnMovable.some(movable => movable.XY.x == kikiXY.x && movable.XY.y == kikiXY.y)) {
                        returnMovable.push(kiki);
                    }
                    if (this.ban.exists(kikiXY) && this.ban.getDirection(kikiXY) == originalKoma.direction) {
                        search(kikiXY);
                    }
                }
            }

            for (const kiki of originalKoma.getMovableVirtual(originalKoma.species, originalKoma.direction, {'ignoreOu': flags.ignoreOu})) {
                const kikiXY = kiki.XY;
                if (arrayChecked.some(checked => checked.x == kikiXY.x && checked.y == kikiXY.y)) {
                    //チェック済みリストに入っていたらそこは調べない。
                    continue;
        ...
        for (let species of this.ban.komaSuggest(this)) {
            arrayMovable = arrayMovable.concat(
                this.getMovableVirtual(species,
                    this.direction,
                    {ignoreOu, reverse}));
        }
        return arrayMovable;
    ...
    public getMovableVirtual(species: Species, direction: Direction, flags: Flags = {}): Kiki[] {
        let arrayKiki: Kiki[] = [];
            ...
            typeLoop: for (let type of [].concat(move['type'])) {
                ...
                if (empties = this.getDataByType(species, "mustBeEmpty", type)) {
                    ...
                    for (let empty of empties) {
                        const emptyXY = this.XY.getCloneRel(vec.getClone((vec.x > 0 ? 1 : -1) * empty['move'][0], (vec.y > 0 ? 1 : -1) * empty['move'][1]));
                ...
                if (empties = this.getDataByType(species, "mustNotBeEmpty", type)) {
                    ...
                    for (let empty of empties) {
                        const v = vec.getClone((vec.x > 0 ? 1 : -1) * empty['move'][0], (vec.y > 0 ? 1 : -1) * empty['move'][1]);
        ...
        arrayKiki = arrayKiki.filter((kiki) => arrayDenyHash.indexOf(kiki['XY'].getHash()) === -1);
...
class JizaitennoKoma extends Koma {
    ...
    getMovable(): Kiki[] {
        ...
        for (let koma of this.ban.getIterator()) {
            if (koma.isNull() || (koma instanceof JizaitennoKoma) || this.isFriend(koma)) continue;
